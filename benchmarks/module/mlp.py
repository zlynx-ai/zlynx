from __future__ import annotations

import argparse
import statistics
import sys
import time
from pathlib import Path
from typing import Callable

import jax
import jax.numpy as jnp
from flax import nnx

ROOT = Path(__file__).resolve().parents[2]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from zlynx.module.mlp import MLP, MLP_N


def parse_dtype(value: str) -> jnp.dtype:
    if value == "float32":
        return jnp.float32
    if value == "bfloat16":
        return jnp.bfloat16
    if value == "float16":
        return jnp.float16
    raise argparse.ArgumentTypeError(
        "dtype must be one of: float32, bfloat16, float16"
    )


def make_input(
    *,
    batch_size: int,
    seq_len: int,
    hidden_size: int,
    dtype: jnp.dtype,
) -> jax.Array:
    key = jax.random.key(0)
    return jax.random.normal(key, (batch_size, seq_len, hidden_size), dtype=dtype)


def block_until_ready(value):
    return jax.tree.map(
        lambda x: x.block_until_ready() if hasattr(x, "block_until_ready") else x,
        value,
    )


def benchmark(
    name: str,
    fn: Callable[[], jax.Array],
    *,
    warmup: int,
    repeat: int,
) -> dict[str, float | str]:
    for _ in range(warmup):
        block_until_ready(fn())

    times = []
    for _ in range(repeat):
        start = time.perf_counter()
        block_until_ready(fn())
        times.append((time.perf_counter() - start) * 1000)

    return {
        "name": name,
        "mean_ms": statistics.fmean(times),
        "median_ms": statistics.median(times),
        "min_ms": min(times),
        "max_ms": max(times),
    }


def format_result(result: dict[str, float | str], baseline_ms: float | None = None) -> str:
    speedup = ""
    if baseline_ms is not None:
        speedup = f" | speedup={baseline_ms / result['mean_ms']:.2f}x"

    return (
        f"{result['name']:<14} "
        f"mean={result['mean_ms']:.3f} ms | "
        f"median={result['median_ms']:.3f} ms | "
        f"min={result['min_ms']:.3f} ms | "
        f"max={result['max_ms']:.3f} ms"
        f"{speedup}"
    )


def run(args: argparse.Namespace) -> None:
    dtype = parse_dtype(args.dtype)
    param_dtype = parse_dtype(args.param_dtype)
    inputs = make_input(
        batch_size=args.batch_size,
        seq_len=args.seq_len,
        hidden_size=args.hidden_size,
        dtype=dtype,
    )

    pallas_mlp = MLP(
        args.hidden_size,
        args.intermediate_size,
        act_fn=args.act_fn,
        bias=args.bias,
        dtype=dtype,
        param_dtype=param_dtype,
        rngs=nnx.Rngs(args.seed),
    )
    normal_mlp = MLP_N(
        args.hidden_size,
        args.intermediate_size,
        act_fn=args.act_fn,
        bias=args.bias,
        dtype=dtype,
        param_dtype=param_dtype,
        rngs=nnx.Rngs(args.seed),
    )

    @nnx.jit
    def run_pallas(module, x):
        return module(x)

    @nnx.jit
    def run_normal(module, x):
        return module(x)

    def pallas_fn():
        return run_pallas(pallas_mlp, inputs)

    def normal_fn():
        return run_normal(normal_mlp, inputs)

    print("=== MLP benchmark ===")
    print(f"device       : {jax.devices()[0].platform} / {jax.devices()[0]}")
    print(f"batch x seq  : {args.batch_size} x {args.seq_len}")
    print(f"hidden       : {args.hidden_size}")
    print(f"intermediate : {args.intermediate_size}")
    print(f"activation   : {args.act_fn}")
    print(f"dtype        : {args.dtype}")
    print(f"warmup/repeat: {args.warmup}/{args.repeat}")
    print()

    results = []
    for name, fn in (("pallas", pallas_fn), ("normal", normal_fn)):
        try:
            result = benchmark(name, fn, warmup=args.warmup, repeat=args.repeat)
        except Exception as exc:
            print(f"{name:<14} failed: {type(exc).__name__}: {exc}")
            continue
        results.append(result)

    if not results:
        raise SystemExit(1)

    baseline_ms = next(
        (result["mean_ms"] for result in results if result["name"] == "normal"),
        None,
    )
    for result in results:
        print(format_result(result, baseline_ms if result["name"] != "normal" else None))


def main() -> None:
    parser = argparse.ArgumentParser(description="Compare MLP Pallas vs normal MLP_N.")
    parser.add_argument("--batch-size", type=int, default=1)
    parser.add_argument("--seq-len", type=int, default=128)
    parser.add_argument("--hidden-size", type=int, default=512)
    parser.add_argument("--intermediate-size", type=int, default=2048)
    parser.add_argument("--act-fn", default="silu")
    parser.add_argument("--bias", action="store_true")
    parser.add_argument("--dtype", default="bfloat16")
    parser.add_argument("--param-dtype", default="float32")
    parser.add_argument("--warmup", type=int, default=5)
    parser.add_argument("--repeat", type=int, default=50)
    parser.add_argument("--seed", type=int, default=42)
    args = parser.parse_args()
    run(args)


if __name__ == "__main__":
    main()
